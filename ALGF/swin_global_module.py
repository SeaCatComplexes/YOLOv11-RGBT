...
class SwinGGMoudle(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        """
        # 投影到嵌入空间 + 添加位置编码
        x1 = self.proj(x1) + self.pos_embed  # [B, C, H, W]
        x2 = self.proj(x2) + self.pos_embed
...
if __name__ == "__main__":
    ...
    # 创建模拟输入 (PET 和 CT)
    x1 = torch.randn(1, 1, 64, 64).to(device)  # e.g., PET
    x2 = torch.randn(1, 1, 64, 64).to(device)  # e.g., CT

    # 前向传播
    output = gg_module(x1, x2)

    print("✅ SwinGGMoudle 测试通过！")
    print("输入 x1 shape:", x1.shape)
    print("输入 x2 shape:", x2.shape)
    print("输出 shape:", output.shape)

    # 统计参数量
    total_params = sum(p.numel() for p in gg_module.parameters())
